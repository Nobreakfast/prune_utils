...
if __name__ == "__main__":
    ...
    if args.restore != 0.0:
        print("restoring")
        if args.restore == 1:
            w_0 = model.module_list[0].conv.weight
            spec_0 = torch.linalg.norm(w_0.view(w_0.shape[0], -1), ord=2)
            lv_0 = spec_0 / (w_0.std() * torch.sqrt(0.5 * torch.tensor(w_0.shape[0])))
            w_1 = model.module_list[1].conv.weight
            spec_1 = torch.linalg.norm(w_1.view(w_1.shape[0], -1), ord=2)
            lv_1 = spec_1 / (w_1.std() * torch.sqrt(0.5 * torch.tensor(w_1.shape[0])))
            w_2 = model.module_list[2].conv.weight
            spec_2 = torch.linalg.norm(w_2.view(w_2.shape[0], -1), ord=2)
            lv_2 = spec_2 / (w_2.std() * torch.sqrt(0.5 * torch.tensor(w_2.shape[0])))
            w_3 = model.fc.weight
            spec_3 = torch.linalg.norm(w_3.view(w_3.shape[0], -1), ord=2)

            model.module_list[0].conv.weight.data /= spec_0
            model.module_list[0].bn.weight.data /= lv_0
            model.module_list[1].conv.weight.data /= spec_1
            model.module_list[1].bn.weight.data /= lv_0 * lv_1
            model.module_list[2].conv.weight.data /= spec_2
            model.module_list[2].bn.weight.data /= lv_0 * lv_1 * lv_2
            model.fc.weight.data /= spec_3
            if args.prune != 0:
                model.module_list[0].conv.weight_orig.data /= spec_0
                model.module_list[1].conv.weight_orig.data /= spec_1
                model.module_list[2].conv.weight_orig.data /= spec_2
                model.fc.weight_orig.data /= spec_3
            ...
            spec_3 = torch.linalg.norm(w_3.view(w_3.shape[0], -1), ord=2)
            model.module_list[0].conv.weight.data /= spec_0
            model.module_list[0].bn.weight.data /= spec_0
            model.module_list[1].conv.weight.data /= spec_1
            model.module_list[1].bn.weight.data /= spec_1
            model.module_list[2].conv.weight.data /= spec_2
            model.module_list[2].bn.weight.data /= spec_2
            model.fc.weight.data /= spec_3
            if args.prune != 0:
                model.module_list[0].conv.weight_orig.data /= spec_0
                model.module_list[1].conv.weight_orig.data /= spec_1
